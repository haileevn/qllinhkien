...
export default function LocationsPage() {
  ...
  const hierarchicalAddParents = buildHierarchyOptions(allLocations);
  const filteredAddParents = addParentSearch.trim()
    ? hierarchicalAddParents.filter(
        (p) =>
          p.name.toLowerCase().includes(addParentSearch.toLowerCase()) ||
          (p.code && p.code.toLowerCase().includes(addParentSearch.toLowerCase())) ||
          p.path.toLowerCase().includes(addParentSearch.toLowerCase())
      )
    : hierarchicalAddParents;
  const selectedAddParentInfo = hierarchicalAddParents.find((p) => p.id === newParentId);
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300">
                    Vị trí cha (Parent Location / Cây thư mục)
                  </label>
                  {hierarchicalAddParents.length > 5 && (
                    <span className="text-[11px] text-slate-400 font-normal">
                      ({hierarchicalAddParents.length} vị trí khả dụng)
                    </span>
                  )}
                </div>

                {hierarchicalAddParents.length > 7 && (
                  <input
                    type="text"
                    placeholder="🔍 Tìm nhanh kho cha..."
                ...
                {selectedAddParentInfo ? (
                  <div className="p-2 rounded-xl bg-sky-50 dark:bg-sky-950/40 border border-sky-200/60 dark:border-sky-800/40 text-[11px] text-sky-800 dark:text-sky-300 flex items-center gap-1.5">
                    ...
                    <span>
                      <strong>Đường dẫn cha:</strong> {selectedAddParentInfo.path}
                    </span>
                  </div>
                ) : (
                  <p className="text-[11px] text-slate-400">
                    💡 Đang chọn: <strong>Kho gốc độc lập</strong> (cấp cao nhất trên sơ đồ cây).
                  </p>
                )}
              </div>
