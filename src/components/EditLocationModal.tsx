...
export default function EditLocationModal({
  ...
  const hierarchicalParents = buildHierarchyOptions(allLocations, forbiddenParentIds);
  const [parentSearch, setParentSearch] = useState('');

  const filteredParents = parentSearch.trim()
    ? hierarchicalParents.filter(
        (p) =>
          p.name.toLowerCase().includes(parentSearch.toLowerCase()) ||
          (p.code && p.code.toLowerCase().includes(parentSearch.toLowerCase())) ||
          p.path.toLowerCase().includes(parentSearch.toLowerCase())
      )
    : hierarchicalParents;

  const selectedParentInfo = hierarchicalParents.find((p) => p.id === parentId);
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                ...
                <span>Vị trí cha (Parent Location / Cây thư mục)</span>
              </label>
              {hierarchicalParents.length > 5 && (
                <span className="text-[11px] text-slate-400 font-normal">
                  ({hierarchicalParents.length} vị trí khả dụng)
                </span>
              )}
            </div>

            {hierarchicalParents.length > 7 && (
              <input
                type="text"
            ...
            {selectedParentInfo ? (
              <div className="p-2 rounded-xl bg-sky-50 dark:bg-sky-950/40 border border-sky-200/60 dark:border-sky-800/40 text-[11px] text-sky-800 dark:text-sky-300 flex items-center gap-1.5">
                ...
                <span>
                  <strong>Đường dẫn cha:</strong> {selectedParentInfo.path}
                </span>
              </div>
            ) : (
